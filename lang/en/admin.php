<?php
return
    [
        'unassigned' => 'Unassigned',
        'created_at' => 'Created At',
        "nav" => [
            "review_application" => "Application Management",
            "category" => "Category Management",
            "support" => "Support Management",
            "human_resource" => "Human Resources",
            'agency' => 'Partner Management',
            "service" => 'Service Management',
            'user' => 'Customer Management',
            'transaction' => 'Transaction Management',
            'marketing' => 'Marketing Management',
        ],
        "user" => [
            "label" => "Technician"
        ],
        "common" => [
            'yes' => 'Yes',
            'no' => 'No',
            'back' => 'Back',
            'minute' => 'minute',
            'hour' => 'hour',
            'day' => 'day',
            'week' => 'week',
            'month' => 'month',
            'year' => 'year',
            'date' => 'Date',
            'count' => 'Count',
            'select_placeholder' => 'Select a value',
            'placeholder' => [
                'select' => 'Select',
            ],
            'affiliate_qr' => 'Affiliate QR',
            'count_collect' => 'Limit Collect',
            'wallet' => 'Wallet',
            'balance' => 'Balance',
            'frozen_balance' => 'Frozen Balance',
            'total_deposit' => 'Total Income from System',
            'total_income' => 'Total Income',
            'total_withdrawal' => 'Total Withdrawal',
            'table' => [
                'id' => 'ID',
                'name' => 'Name',
                'username' => 'Username',
                'email' => 'Email',
                'address' => 'Address',
                'created_at' => 'Created At',
                'updated_at' => 'Updated At',
                'deleted_at' => 'Deleted At',
                'avatar' => 'Avatar',
                'phone' => 'Phone Number',
                'date_of_birth' => 'Date of Birth',
                'gender' => 'Gender',
                'status' => 'Status',
                'language' => 'Language',
                'status_review' => 'Review Status',
                'last_login' => 'Last Login',
                'basic_info' => 'Basic Info',
                'account_info' => 'Account Info',
                'role' => 'Role',
                'password' => 'Password',
                'password_desc' => 'Leave blank to keep current password',
                'bio' => 'Bio',
                'is_online' => 'Online Status',
                'description' => 'Description',
                'image' => 'Image',
                'position' => 'Position',
                'is_featured' => 'Featured',
                'usage_count' => 'Usage Count',
                'effective_date' => 'Effective Date',
                'application_date' => 'Application Date',
                'creator' => 'Created By',
                'performed_count' => 'Performed Count',
            ],
            'action' => [
                'view' => 'View',
                'edit' => 'Edit',
                'delete' => 'Delete',
                'restore' => 'Restore',
                'force_delete' => 'Force Delete',
                'confirm_delete' => 'Confirm Delete',
                'export_excel' => 'Export Data',
                'import' => 'Import Data',
                'upload_excel' => 'Upload .xlsx (No Product Info)',
                'add' => 'Add New',
                'download_template' => 'Download Template',
                'save' => 'Save',
                'cancel' => 'Cancel',
                'book' => 'Book',
                'close' => 'Close',
                'create' => 'Create New',
                'detail' => 'Details',
                'view_service' => 'View Service',
                'buff_service' => 'Buff Service Usage',
                'view_ktv_manager_agency' => 'View Managed KTVs',
                'agency_dashboard' => 'Partner Dashboard',
                'ktv_dashboard' => 'KTV Dashboard',
            ],
            'tooltip' => [
                'view' => 'View Details ',
                'edit' => 'Edit ',
                'delete' => 'Delete This',
                'restore' => 'Restore Deleted',
                'force_delete' => 'Permanently Delete From System',
                'cant_not_save_review_application' => 'Cannot save while reviewing application',
            ],
            'modal' => [
                'delete_title' => 'Delete ',
                'delete_confirm' => 'Are you sure you want to delete selected items?',
            ],
            'gender' => [
                'male' => 'Male',
                'female' => 'Female',
            ],
            'filter' => [
                'gender' => 'Gender',
                'status' => 'Status',
                'is_featured' => 'Featured',
                'type' => 'Type',
                'review_status' => 'Review Status',
                'from_date' => 'From Date',
                'to_date' => 'To Date',
                'from' => 'From',
                'to' => 'To',
                'is_leader' => 'Is Leader',
            ],
            'status' => [
                'active' => 'Active',
                'inactive' => 'Inactive',
                'pending' => 'Pending',
                'confirmed' => 'Confirmed',
            ],
            'empty' => 'No Data',
            'form' => [
                'name' => 'Name',
                'name_vi' => 'Name (Vietnamese)',
                'name_en' => 'Name (English)',
                'name_cn' => 'Name (Chinese)',
                'position' => 'Position',
                'description' => 'Description',
                'description_vi' => 'Description (Vietnamese)',
                'description_en' => 'Description (English)',
                'description_cn' => 'Description (Chinese)',
                'image' => 'Image',
                'is_featured' => 'Featured',
                'is_active' => 'Active',
                'duration' => 'Duration',
                'price' => 'Price',
                'add_price' => 'Add Price',
            ],
            'support_category' => [
                'label' => 'Support category',
                'section' => [
                    'general' => 'General settings',
                    'content' => 'Multilingual content',
                ],
                'helper' => [
                    'position' => 'Lower numbers are shown first.',
                ],
                'table' => [
                    'id' => 'ID',
                    'name' => 'Category name',
                    'description' => 'Description',
                    'position' => 'Position',
                    'status' => 'Status',
                    'created_at' => 'Created at',
                    'updated_at' => 'Updated at',
                ],
                'empty_state' => [
                    'heading' => 'No support categories yet',
                    'description' => 'Create the first support category so mobile can use it when creating tickets.',
                ],
            ],
            'unit' => [
                'user' => 'Users',
                'point' => 'Points',
            ],
            'currency' => 'Points',
            'from' => 'From',
            'to' => 'To',
            'error' => [
                'required' => 'This field is required',
                'numeric' => 'Please enter only numbers',
            ],
            'contract_file_type' => [
                'term_of_use' => 'Terms of Use',
                'policy_register' => 'Registration Policy',
                'policy_privacy' => 'Privacy Policy',
                'policy_for_ktv' => "Technician's Operating Policy",
                'policy_for_agency' => "Partner's Operating Policy",
                'face_with_identity_card' => 'Front and Back of Identity Card',
            ],

        ],
        "ktv" => [
            "label" => "KTV Management",
            "label_leader" => "Technician Leader Management",
            "model_label" => "Technician",
            "action" => [
                "view_services" => "View Services",
                'choose_categories' => 'Choose Categories',
            ],
            "tooltip" => [
                "view_services" => "View KTV Services List"
            ],
            "modal" => [
                "services_title" => "Services of :name",
                "services_created" => "Created Services",
                "services_booked" => "Booked Services"
            ],
            'messages' => [
                'max_images_limit' => 'Max :max images. You currently have :current and are adding :new.',
                'upload_success' => 'Image uploaded successfully.',
                'delete_success' => 'Image deleted successfully.',
            ],
            'infolist' => [
                'label_tab' => 'Statistics',
                'info_label' => 'Technician Info',
                'is_leader' => 'Is Leader?',
                'wallet' => 'Wallet',
                'balance' => 'Balance',
                'total_deposit' => 'Total Deposit',
                'total_withdrawal' => 'Total Withdrawal',
                'total_income' => 'Total Income from Services',
                'total_income_desc' => 'Total income from all completed services.',
                'received_income' => 'Actual Income Received',
                'received_income_desc' => 'Actual income received from all completed services after discount deduction.',
                'total_customers' => 'Number of Customers Serviced',
                'total_customers_desc' => 'Total number of customers who booked services from this technician.',
                'total_bookings' => 'Total Bookings',
                'total_bookings_desc' => 'Total number of bookings received by the technician.',
                'affiliate_income' => 'Affiliate Income',
                'affiliate_income_desc' => 'Amount received from referring customers.',
                'total_reviews' => 'Total Reviews',
                'total_reviews_desc' => 'Total number of reviews received by the technician.',
                'list_referral_leader' => 'List of Referred KTVs',
                'transaction_list' => 'Transaction List',
                'booking_list' => 'Booking List',
                'customer_affiliate' => 'Customer Affiliate',
            ],
            'relation_manager' => [
                'reviews_received' => 'Reviews Received',
            ],
            'empty_state' => [
                'heading' => 'No KTVs found',
            ],
        ],
        "ktv_technique" => [
            "acupressure" => "Acupressure",
            "massage" => "Massage",
            "therapy" => "Deep Therapy",
            "stretching" => "Muscle Stretching",
            "essential_oil" => "Essential Oil Relaxation",
        ],
        "ktv_service_location" => [
            "home" => "Private Home",
            "hotel" => "Hotel",
        ],
        "ktv_apply" => [
            "nav" => "KTV Application",
            "label" => "KTV Application",
            "status" => [
                "pending" => "Pending",
                "approved" => "Approved",
                "rejected" => "Rejected",
            ],
            "file_type" => [
                "identity_card_front" => "ID Card Front",
                "identity_card_back" => "ID Card Back",
                "license" => "Degree/Certificate",
                "certificate" => "Professional Degree/Certificate",
                "health_insurance" => "Health Insurance",
                "business_image" => "Business Image",
                "ktv_image_display" => "Customer Display Image (min :min, max :max)",
                "face_with_identity_card" => "Face with ID Card",
            ],
            "fields" => [
                "experience" => "Experience",
                "skills" => "Skills",
                "bio" => "Self Introduction",
                "experience_desc_vi" => "Experience Description (Vietnamese)",
                "experience_desc_en" => "Experience Description (English)",
                "experience_desc_cn" => "Experience Description (Chinese)",
                "province" => "Province/City",
                "address" => "Address Details",
                "address_search" => "Search Address",
                "files" => "Attached Files",
                "file_type" => "File Type",
                "file_name" => "File Name",
                "years" => "years",
                "system_info" => "System Info",
                "registration_info" => "Registration Info",
                "personal_info" => "Personal Info",
                'agency' => 'Partner',
                'latitude' => 'Latitude',
                'longitude' => 'Longitude',
                'schedule' => 'Schedule',
                'is_working' => 'Ready to work',
                'is_working_helper' => 'Quick toggle online status',
                'working_schedule' => 'Weekly Schedule Config',
                'day_key' => 'Day',
                'monday' => 'Monday',
                'tuesday' => 'Tuesday',
                'wednesday' => 'Wednesday',
                'thursday' => 'Thursday',
                'friday' => 'Friday',
                'saturday' => 'Saturday',
                'sunday' => 'Sunday',
                'start_time' => 'Start Time',
                'end_time' => 'End Time',
                'is_cross_day' => 'Cross Day',
                'is_leader' => 'Leader',
                'is_priority' => 'Priority Display',
                'nickname' => 'Nickname',
                'contact_phone' => 'Dispatch Contact Phone',
                'contact_verified' => 'Contact Verified',
                'portrait_verified' => 'Portrait Verified by MasaHome',
                'portrait_verified_at' => 'Portrait Verification Time',
                'certificate_verified' => 'Certificate Verified',
                'certificates' => 'Professional Certificates',
                'techniques' => 'Specialized Techniques',
                'strength_service_ids' => 'Strength Services (Max 3)',
                'district' => 'District',
                'ward' => 'Ward',
                'priority_areas' => 'Priority Operating Areas',
                'service_locations' => 'Service Locations',
                'verification_section' => 'MasaHome Verification Status',
                'expertise_section' => 'Techniques & Strength Services',
                'area_section' => 'Operating Areas & Service Locations',
                'work_province' => 'Province / City',
                'work_province_placeholder' => 'e.g.: Ho Chi Minh City',
                'work_wards' => 'Ward / Commune',
                'work_wards_placeholder' => 'e.g.: Ben Nghe Ward, Ben Thanh Ward',
                'select_province_first' => 'Please select Province / City first',
                'is_online' => 'ON/OFF Order Reception Status',
                'certificates_upload' => 'KTV Professional Certificates',
            ],
            "actions" => [
                "approve" => [
                    "label" => "Approve Application",
                    "heading" => "Approve KTV Application",
                    "description" => "Are you sure you want to approve this application?",
                    "success_title" => "Approval Successful",
                    "success_body" => "KTV application has been approved.",
                    "error_title" => "Approval Failed",
                ],
                "reject" => [
                    "label" => "Reject Application",
                    "heading" => "Reject KTV Application",
                    "description" => "Are you sure you want to reject this application?",
                    "reason_label" => "Rejection Reason",
                    "success_title" => "Rejection Successful",
                    "success_body" => "KTV application has been rejected.",
                ],
            ],
        ],
        "user_role" => [
            "customer" => "Customer",
            "ktv" => "Technician",
            "agency" => "Partner",
            "admin" => "Admin",
        ],
        'category' => [
            'label' => 'Category',
            'basic_info' => 'Basic Info',
            'price_config' => 'Price Config',
            'price_list' => 'Price List',
        ],
        'notification' => [
            'success' => [
                'update_success' => 'Update Successful',
                'create_success' => 'Creation Successful',
            ],
            'error' => [
                'update_error' => 'Update Failed',
                'create_error' => 'Creation Failed',
            ],
        ],
        'agency' => [
            'label' => 'Partner',
            'infolist' => [
                'label_tab' => 'Statistics',
                'info_label' => 'Technician Info',
                'is_leader' => 'Is Leader?',
                'wallet' => 'Wallet',
                'balance' => 'Balance',
                'total_deposit' => 'Total Deposit',
                'total_withdrawal' => 'Total Withdrawal',
                'total_profit_referral_ktv' => 'Total KTV Referral Profit',
                'total_profit_referral_ktv_desc' => 'Total profit from KTV service referrals.',
                'total_profit_affiliate' => 'Total Affiliate Profit',
                'total_profit_affiliate_desc' => 'Total profit from customer referrals.',
                'total_referral_customer' => 'Total Referred Customers',
                'total_referral_customer_desc' => 'Total number of referred customers.',
                'total_customer_order_ktv' => 'Total Customer Orders via KTV',
                'total_customer_order_ktv_desc' => 'Total customer orders from managed KTVs.',
                'total_customer_affiliate_order' => 'Total Affiliate Customer Orders',
                'total_customer_affiliate_order_desc' => 'Total orders from customers referred by this partner.',
                'list_referral_leader' => 'Referral List',
                'transaction_list' => 'Transaction List',
                'affiliate_qr' => 'Affiliate QR Code',
            ]
        ],
        'agency_apply' => [
            'label' => 'Partner Registration',
            'fields' => [
                'name' => 'Name',
                'email' => 'Email',
                'phone' => 'Phone',
                'address' => 'Address',
                'address_search' => 'Search Address',
                'province' => 'Province/City',
                'files' => 'Files',
                'personal_info' => 'Personal Info',
                'registration_info' => 'Registration Info',
                'system_info' => 'System Info',
                'file_type' => 'File Type',
                'bio_vi' => 'Business Introduction (Vietnamese)',
                'bio_en' => 'Business Introduction (English)',
                'bio_cn' => 'Business Introduction (Chinese)',
                'status' => 'Status',
                'note' => 'Note',
                'latitude' => 'Latitude',
                'longitude' => 'Longitude',
                'identity_card_front' => 'ID Card Front',
                'identity_card_back' => 'ID Card Back',
                'face_with_identity_card' => 'Face with ID Card',
            ],
            'actions' => [
                "approve" => [
                    "label" => "Approve Application",
                    "heading" => "Approve Partner Application",
                    "description" => "Are you sure you want to approve this application?",
                    "success_title" => "Application Approved Successfully",
                    "success_body" => "Partner application has been approved.",
                    "error_title" => "Approval Failed",
                ],
                "reject" => [
                    "label" => "Reject Application",
                    "heading" => "Reject Partner Application",
                    "description" => "Are you sure you want to reject this application?",
                    "reason_label" => "Reason for Rejection",
                    "success_title" => "Application Rejected Successfully",
                    "success_body" => "Partner application has been rejected.",
                ],
            ],
        ],
        'service' => [
            'label' => 'Service',
            'fields' => [
                'name' => 'Name',
                'category' => 'Category',
                'provider' => 'KTV',
                'image' => 'Image',
                'status' => 'Status',
                'description' => 'Description',
                'options' => 'Options',
                'duration' => 'Duration',
                'price' => 'Price',
                'options_avg_price' => 'Average Price',
                'created_at' => 'Created At',
                'updated_at' => 'Updated At',
                'option_category_prices' => 'Service Packages (Price & Time)',
                'no_option_category_prices' => 'No price packages for this category.',
            ],
            'error' => [
                'option_category_prices' => 'Please select at least 1 service package.',
            ],
        ],
        'booking' => [
            'label' => 'Booking',
            'fields' => [
                'staff' => 'Technician',
                'user' => 'Customer',
                'ktv_user' => 'Technician',
                'service' => 'Service',
                'coupon' => 'Coupon',
                'duration' => 'Duration',
                'booking_time' => 'Booking Time',
                'start_time' => 'Start Time',
                'time_range' => 'Start - End',
                'end_time' => 'End Time',
                'price' => 'Price',
                'price_transportation' => 'Price Transportation',
                'price_before_discount' => 'Price Before Discount',
                'payment_type' => 'Payment Method',
                'note' => 'Note',
                'address' => 'Address',
                'latitude' => 'Latitude',
                'longitude' => 'Longitude',
                'rating' => 'Rating',
                'comment' => 'Comment',
                'status' => 'Status',
                'note_address' => 'Address Note',
                'reason_cancel' => 'Cancel Reason',
                'customer_paid_total' => 'Customer Paid Total',
                'pay_to_ktv' => 'Pay to KTV',
                'amount_pay_to_ktv' => 'Amount Pay to KTV',
                'amount_pay_back_to_client' => 'Amount Pay Back to Client',
                'cancel_by' => 'Cancelled By',
                'distance_to_client' => 'Distance to Client',
            ],
            'sections' => [
                'general_info' => 'General Info',
                'detail' => 'Detail Info',
            ],
            'status' => [
                'pending' => 'Pending',
                'confirmed' => 'Confirmed',
                'ongoing' => 'Ongoing',
                'completed' => 'Completed',
                'canceled' => 'Canceled',
                'waiting_cancel' => 'Waiting Cancel',
                'payment_failed' => 'Payment Failed',
                'waiting_ktv_confirm' => 'Waiting KTV Confirm',
                'open_for_application' => 'Open For Application',
            ],
            'application_status' => [
                'applied' => 'Applied',
                'selected' => 'Selected',
                'rejected' => 'Rejected',
                'removed' => 'Removed',
                'expired' => 'Expired',
            ],
            'payment_type' => [
                'qr_banking' => 'QR Banking',
                'zalo_pay' => 'Zalo Pay',
                'momo_pay' => 'Momo Pay',
                'withdrawal' => 'Withdrawal',
                'by_points' => 'By Points',
                'refund' => 'Refund',
                'wechat' => 'WeChat Pay',
            ],
            'actions' => [
                'view' => [
                    'label' => 'View Details',
                    'heading' => 'View Info',
                    'description' => 'View details',
                ],
                'cancel' => [
                    'heading' => 'Confirm Cancel Booking',
                    'description' => 'Are you sure you want to cancel this booking?',
                ],
                'view_location' => 'View Location',
                'view_map' => 'View Google Map',
                'confirm_cancel' => 'Confirm Cancel',
                'confirm_cancel_helper_text' => 'If refund amount is not set, full amount will be refunded to customer.',
                'cancel_success_title' => 'Booking cancelled successfully',
                'cancel_error_title' => 'Booking cancellation failed, error occurred',
                'cancel_processing_title' => 'Cancel booking is processing',
                'reassign' => [
                    'label' => 'Reassign KTV',
                    'heading' => 'Reassign to another KTV',
                    'description' => 'Select a suitable KTV to reassign this booking. This action will cancel the current booking and create a new one for the selected KTV.',
                    'select_service_placeholder' => 'Select KTV and Alternative Service',
                    'success_title' => 'Reassignment Successful',
                    'processing_body' => 'Reassignment request is being processed',
                    'error_title' => 'Reassignment Failed',
                    'modal_submit' => 'Confirm Reassign',
                    'processing_title' => 'Reassignment request is being processed',
                ],
            ],
            'success' => [
                'cancel' => 'Booking cancelled successfully',
            ],
        ],
        'setting' => [
            'label' => 'System Settings',
            'label_setting' => 'System',
            'label_config_discount_rate' => 'Discount',
            'label_mobile_app' => 'Mobile App',
            'label_fee' => 'Fees - System Operation',
            'validation' => [
                'version' => 'Version must look like 1, 1.2, or 1.2.3',
                'url' => 'The URL is invalid',
                'mobile_maintenance' => 'Mobile maintenance must be 0 or 1',
            ],
            'section' => [
                'system_config' => 'System Config',
                'affiliate_config' => 'Affiliate Config',
            ],
            'fields' => [
                'payos_client_id' => 'PayOS Client ID',
                'payos_api_key' => 'PayOS API Key',
                'payos_checksum_key' => 'PayOS Checksum Key',
                'goong_api_key' => 'Goong API Key',
                'goong_maptiles_key' => 'Goong Maptiles Key',
                'goong_maptiles_key_helper' => 'Key used to render Goong maps on mobile. Do not use the REST Places/Directions key here.',
                'gemini_api_key' => 'Gemini API Key',
                'currency_exchange_rate' => 'Exchange Rate (VND -> Points)',
                'commission_rate' => 'Commission Rate',
                'min_commission' => 'Min Commission',
                'max_commission' => 'Max Commission',
                'target_role' => 'Applicable Role',
                'is_active' => 'Active',
                'break_time_gap' => 'Gap Between Shifts',
                'auto_finish_ongoing_booking_minutes' => 'Auto-finish ongoing booking after',
                'auto_finish_ongoing_booking_minutes_helper' => 'Additional overdue minutes after service duration before the system auto-finishes an ongoing booking',
                'booking_application_timeout_minutes' => 'Auto-cancel open applications after',
                'booking_application_timeout_minutes_helper' => 'Minutes customers/technicians have to accept or select an applicant before auto-cancel and refund',
                'discount_rate' => 'Discount Rate',
                'discount_rate_helper' => 'Formula: P(ktv) = P(transaction) - (P(transaction) * (R(discount) / 100))',
                'discount_rate_referrer_agency' => 'Discount Rate for Referral Agencies',
                'discount_rate_referrer_agency_helper' => 'Discount rate for agency on 1 completed order of 1 referred KTV %',
                'discount_rate_referrer_ktv' => 'Discount Rate for Referral KTVs',
                'discount_rate_referrer_ktv_helper' => 'Discount rate for technician on 1 completed order of 1 referred KTV %',
                'discount_rate_referrer_ktv_leader' => 'Discount Rate for Referral KTV Leaders',
                'discount_rate_referrer_ktv_leader_helper' => 'Discount rate for leader technician on 1 completed order of 1 referred KTV %',
                'ktv_leader_min_referrals' => 'Number of KTV referrals to become a leader',
                'ktv_leader_min_referrals_helper' => 'When a KTV refers this number of KTVs (approved), they will automatically be marked as a KTV leader',
                'ktv_referral_reward_amount' => 'Referral Reward Amount',
                'ktv_referral_reward_amount_helper' => 'Amount received for successful KTV referral. Set currently to 0 to disable.',
                'ios_min_supported_version' => 'Minimum Supported iOS Version',
                'ios_latest_version' => 'Latest iOS Version',
                'android_min_supported_version' => 'Minimum Supported Android Version',
                'android_latest_version' => 'Latest Android Version',
                'appstore_url' => 'App Store URL',
                'chplay_url' => 'Google Play URL',
                'mobile_maintenance' => 'Mobile Maintenance',
                'mobile_maintenance_helper' => 'Enter 1 to enable maintenance, 0 to disable',
                'sp_zalo' => 'Zalo Support Page',
                'sp_facebook' => 'Facebook Support Page',
                'sp_phone' => 'Support Phone',
                'sp_wechat' => 'Wechat Support Link',
                'sp_wechat_qr_image' => 'Wechat Pay QR Image/URL',
                'zalo_merchant_id' => 'Zalo Merchant ID',
                'zalo_merchant_id_helper' => 'Zalo Merchant ID supports Zalo Pay',
                'zalo_merchant_key_1' => 'Zalo Merchant Key 1',
                'zalo_merchant_key_1_helper' => 'Zalo Merchant Key 1 supports Zalo Pay',
                'zalo_merchant_key_2' => 'Zalo Merchant Key 2',
                'zalo_merchant_key_2_helper' => 'Zalo Merchant Key 2 supports Zalo Pay',
                'zalo_app_id' => 'Zalo App ID',
                'zalo_app_id_helper' => 'Zalo App ID supports Zalo Pay',
                'zalo_appsecret_key' => 'Zalo Appsecret Key',
                'zalo_appsecret_key_helper' => 'Zalo Appsecret Key supports Zalo Pay',
                'zalo_oa_id' => 'Zalo OA ID',
                'zalo_oa_id_helper' => 'Zalo OA ID supports Zalo Pay',
                'zalo_template_id' => 'Zalo Template ID',
                'zalo_template_id_helper' => 'Zalo Template ID supports Zalo Pay',
                'price_transportation' => 'Transportation Fee',
                'price_transportation_helper' => 'Transportation fee per km (crow flies) for KTV, paid by customer',
                'exchange_rate_vnd_cny' => 'Exchange Rate VND/CNY ~ 1 CNY = X VND',
                'exchange_rate_vnd_cny_helper' => 'The system will automatically update this exchange rate',
                'fee_withdraw_percentage' => 'Withdrawal Fee %',
                'fee_withdraw_percentage_helper' => 'Withdrawal Fee % per withdrawal',
            ],
            'actions' => [
                'save' => 'Save Changes',
            ],
        ],
        'transaction' => [
            'label' => 'Transaction',
            'fields' => [
                'user' => 'User',
                'code' => 'Transaction Code',
                'type' => 'Transaction Type',
                'amount' => 'Amount',
                'status' => 'Status',
                'created_at' => 'Transaction Time',
            ],
            'empty_state' => [
                'heading' => 'No transactions found',
            ],
            'pending' => [
                'label' => 'Pending Transactions',
                'empty_state' => [
                    'heading' => 'No pending transactions',
                ],
            ],
            'type' => [
                'DEPOSIT_QR_CODE' => 'Deposit (QR)',
                'DEPOSIT_ZALO_PAY' => 'Deposit (ZaloPay)',
                'DEPOSIT_MOMO_PAY' => 'Deposit (Momo)',
                'WITHDRAWAL' => 'Withdraw',
                'PAYMENT' => 'Payment',
                'AFFILIATE' => 'Commission',
                'PAYMENT_FOR_KTV' => 'Payment for KTV',
                'REFUND' => 'Refund',
                'RETRIEVE_PAYMENT_REFUND_KTV' => 'Retrieve Payment from KTV (Refund)',
                'REFERRAL_KTV' => 'KTV Referral Commission',
                'REFERRAL_INVITE_KTV_REWARD' => 'KTV Invitation Reward',
                'DEPOSIT_WECHAT_PAY' => 'Deposit (WeChat Pay)',
                'FEE_WITHDRAW' => 'Withdrawal Fee',
                'PAYMENT_FEE_TRANSPORT' => 'Payment Fee for Transportation',
                'PAYMENT_KTV_EARN_TRANSPORT' => 'Payment Earn for Transportation',
                'REFUND_CUSTOMER_TRANSPORT' => 'Refund Customer for Transportation',
                'PAYMENT_REFUND_KTV_FOR_BOOKING_CANCEL' => 'Refund KTV for Booking Cancel',
                'SUBTRACT_MONEY_DISCOUNT_SERVICE' => 'Subtract Discount for Service',
                'REFUND_MONEY_DISCOUNT_SERVICE' => 'Refund Discount for Service',
            ],
            'status' => [
                'PENDING' => 'Pending',
                'COMPLETED' => 'Success',
                'FAILED' => 'Failed',
                'REFUNDED' => 'Refunded',
                'CANCELLED' => 'Cancelled',
            ],
            'actions' => [
                'approve' => 'Approve',
                'cancel' => 'Cancel',
                'transfer' => 'Transfer Money ',
                'transfer_confirmation_message' => 'Are you sure you want to transfer this transaction?',
                'fee_transfer' => '(Transaction ID: :transaction_id)',
                'approve_confirmation_message' => 'Are you sure you want to approve this transaction?',
                'cancel_confirmation_message' => 'Are you sure you want to cancel this transaction?',
            ],
            'errors' => [
                'withdraw_info_not_found' => 'Withdrawal info not found',
            ],
        ],
        'customer_rank' => [
            'standard' => 'Standard',
            'gold' => 'Gold',
            'vip' => 'VIP',
        ],
        'demand_status' => [
            'need_now' => 'Need Now',
            'exploring' => 'Exploring',
            'booked' => 'Booked',
            'no_need' => 'No Need',
        ],
        'preferred_time_slot' => [
            'night' => '00:00 - 06:00 (Night)',
            'morning' => '06:00 - 12:00 (Morning)',
            'afternoon' => '12:00 - 18:00 (Afternoon)',
            'evening' => '18:00 - 24:00 (Evening)',
        ],
        'language' => [
            'vietnamese' => 'Vietnamese',
            'english' => 'English',
            'chinese' => 'Chinese',
            'japanese' => 'Japanese',
            'korean' => 'Korean',
        ],
        'customer' => [
            'label' => 'Customer',
            'section' => [
                'info' => 'Basic Info',
                'booking_history' => 'Booking History',
                'wallet' => 'Wallet',
                'reviews' => 'Reviews',
                'service_needs' => 'Service Needs & Location',
                'crm_management' => 'CRM Management & CSKH Tier',
            ],
            'fields' => [
                'id' => '#ID',
                'avatar' => 'Avatar',
                'name' => 'Name',
                'email' => 'Email',
                'phone' => 'Phone',
                'password' => 'Password',
                'dob' => 'Date of Birth',
                'role' => 'Role',
                'balance' => 'Wallet Balance',
                'address' => 'Address',
                'created_at' => 'Created At',
                'updated_at' => 'Updated At',
                'languages' => 'Languages Spoken',
                'demand_status' => 'Demand Status',
                'preferred_services' => 'Preferred Services',
                'preferred_techniques' => 'Preferred Techniques',
                'preferred_time_slots' => 'Preferred Time Slots',
                'address_detail' => 'Service Area Address',
                'customer_rank' => 'Customer Rank',
                'assigned_cskh' => 'Assigned CSKH Staff',
                'unassigned' => 'Unassigned',
                'total_spent' => 'Total Spent (VND)',
                'booking_count' => 'Completed Bookings',
                'aov' => 'Average Order Value (AOV)',
                'cskh_notes' => 'CSKH Notes',
                'cskh_notes_helper' => 'Edit CSKH notes and click Save to update.',
                'ward' => 'Ward / Commune',
                'province' => 'Province / City',
                'select_province_first' => 'Please select Province / City first',
            ],
        ],
        'review' => [
            'label' => 'Review',
            'fields' => [
                'reviewer' => 'Reviewer',
                'provider' => 'Reviewed KTV',
                'service' => 'Service',
                'rating' => 'Rating',
                'comment' => 'Comment',
                'hidden' => 'Hidden',
                'is_virtual' => 'Virtual Review',
                'created_at' => 'Created At',
                'updated_at' => 'Updated At',
                'user' => 'Staff',
            ],
            'action' => [
                'virtual' => [
                    'title' => 'Create Virtual Review',
                    'virtual_name' => 'Reviewer Name (Virtual)',
                    'virtual_name_helper_text' => 'Leaving the name blank will default to an anonymous review',
                    'rating' => 'Star Rating',
                    'target_language' => 'Review Language',
                    'target_language_helper_text' => 'Select the review language for the AI to auto-write the review',
                    'comment' => 'Review Content',
                    'comment_hint' => 'You can write it yourself or use the AI button next to it',
                    'comment_helper_text' => 'AI can be used to auto-write the review, or write it yourself. If there is nothing to write, leave it blank.',
                    'generate_ai_comment_tooltip' => 'Use Gemini to auto-generate review based on star rating',
                    'generate_ai_comment_success' => 'Content generated by AI',
                    'generate_ai_comment_error' => 'Error calling AI',
                ]
            ]
        ],
        'wallet_transaction' => [
            'label' => 'Wallet Transaction',
            'fields' => [
                'user' => 'User',
                'code' => 'Transaction Code',
                'type' => 'Transaction Type',
                'amount' => 'Amount',
                'status' => 'Status',
                'created_at' => 'Created At',
            ],
            'type' => [
                'DEPOSIT_QR_CODE' => 'Deposit (QR)',
                'DEPOSIT_ZALO_PAY' => 'Deposit (ZaloPay)',
                'DEPOSIT_MOMO_PAY' => 'Deposit (Momo)',
                'WITHDRAWAL' => 'Withdraw',
                'PAYMENT' => 'Payment',
                'AFFILIATE' => 'Commission',
            ],
            'status' => [
                'PENDING' => 'Pending',
                'COMPLETED' => 'Success',
                'FAILED' => 'Failed',
            ],
            'actions' => [
                'approve' => 'Approve',
                'cancel' => 'Cancel',
            ],
        ],
        'dashboard' => [
            'filters' => [
                'date_range' => 'Date Range',
            ],
            'stats' => [
                'revenue' => 'Total Revenue (Points)',
                'booking_value' => 'Booking Value',
                'new_bookings' => 'New Bookings',
                'new_users' => 'New Users',
                'pending_profiles' => 'Pending Profiles',
                'affiliate_commission' => 'Affiliate Commission',
            ],
            'charts' => [
                'revenue_trend' => 'Revenue Trend',
                'user_activity' => 'User Activity',
                'booking_status' => 'Booking Status',
                'top_services' => 'Top Services',
                'user_roles' => 'User Roles Distribution',
                'review_ratings' => 'Rating Analysis',
                'revenue_deposit' => 'Deposit',
                'revenue_booking' => 'Booking',
                'new_users' => 'New Users',
                'active_users' => 'Active Users',
                'profit_trend' => 'Profit Trend (Paid Orders)',
                'profit' => 'Total Amount (Paid)',
                'transaction_dashboard' => 'Transaction Dashboard',
                'income' => 'Income',
                'expense' => 'Expense',
            ],
            'time' => [
                'today' => 'Today',
                'week' => 'Last 7 Days',
                'month' => 'This Month',
                'year' => 'This Year',
            ],
            'widgets' => [
                'operation_cost' => [
                    'label' => 'Operation Cost',
                    'desc' => 'Total Cost (Withdrawal, KTV, Affiliate)',
                    'primary_order_count' => 'Total Orders',
                    'primary_order_desc' => 'Includes refunded orders',
                    'primary_service_value' => 'Service Revenue',
                    'primary_service_desc' => 'Total Service Value',
                    'canceled_orders' => 'Canceled Orders',
                    'canceled_orders_desc' => 'Orders Canceled',
                    'refund_amount' => 'Total Refunds',
                    'refund_amount_desc' => 'Total Refunded Amount',
                ],
                'add_on' => [
                    'volume' => 'Add-on Volume',
                    'volume_desc' => 'Add-on Order Count (TBD)',
                    'revenue' => 'Add-on Revenue',
                    'revenue_desc' => 'Add-on Revenue (TBD)',
                    'refund_volume' => 'Add-on Refund Volume',
                    'refund_volume_desc' => 'Add-on Refund Count (TBD)',
                    'refund_amount' => 'Add-on Refund Amount',
                    'refund_amount_desc' => 'Add-on Refund Amount (TBD)',
                ],
                'general' => [
                    'order_volume' => 'Order Volume',
                    'order_volume_desc' => 'Total Order Count',
                    'sales' => 'Sales',
                    'sales_desc' => 'Total Sales',
                    'net_sales' => 'Net Sales',
                    'net_sales_desc' => 'Net Sales (Minus Refunds)',
                    'commission' => 'Commission',
                    'commission_desc' => 'Total Commission',
                    'coupon' => 'Coupon',
                    'coupon_desc' => 'Total Coupon Value',
                    'volumn_affiliate' => 'Commission fee',
                    'volumn_affiliate_desc' => 'Total commission paid'
                ],
                'technician_status' => [
                    'total' => 'Total Staff',
                    'total_desc' => 'Total KTVs',
                    'online' => 'Online',
                    'online_desc' => 'KTVs Online',
                    'resting' => 'Resting',
                    'resting_desc' => 'KTVs Resting (Online - Working)',
                    'working' => 'Working',
                    'working_desc' => 'KTVs Serving Customers',
                ],
                'leaderboard' => [
                    'heading' => 'Agency/KTV Leaderboard',
                    'columns' => [
                        'id' => 'ID',
                        'avatar' => 'Avatar',
                        'name' => 'Name',
                        'rating' => 'Rating',
                        'level' => 'Level',
                        'service_duration' => 'Service Duration',
                        'revenue' => 'Revenue',
                        'addon_rate' => 'Add-on Rate',
                        'points' => 'Points',
                        'total_orders' => 'Total Orders',
                    ],
                ],
            ],
        ],
        'coupon' => [
            'label' => 'Coupon',
            'navigation_label' => 'Coupon Management',
            'sections' => [
                'advanced_config' => 'Advanced Configuration',
            ],
            'fields' => [
                'code' => 'Code',
                'discount' => 'Discount',
                'label' => 'Name',
                'type' => 'Type',
                'start_date' => 'Start Date',
                'end_date' => 'End Date',
                'max_uses' => 'Max Uses',
                'is_percentage' => 'Discount Type',
                'description' => 'Description',
                'discount_value' => 'Discount Value (points/%)',
                'max_discount' => 'Max Discount',
                'usage_limit' => 'Usage Limit',
                'used_count' => 'Used Count',
                'is_active' => 'Status',
                'count_collect' => 'Limit Collect',
                'display_ads' => 'Display Ads',
                'valid_hours' => 'Validity hours (for Welcome code)',
                'valid_hours_helper' => 'Time calculated from user registration success. Default 48 hours.',
                'banners' => [
                    'label' => 'Banner',
                    'vi' => 'Vietnamese Banner',
                    'en' => 'English Banner',
                    'cn' => 'Chinese Banner',
                ],
                'config' => [
                    'label' => 'Coupon Config',
                    'per_day_global' => 'Daily Limit (System)',
                    'min_order_value' => 'Minimum Order Value',
                    'used_day' => 'Used Today',
                    'ao' => 'Collected Today',
                    'daily_tracking_helper' => 'System automatically resets this count daily.',
                    'allowed_time_slots' => 'Golden Hours',
                    'time_slots_helper' => 'Leave blank to apply all day.',
                    'add_time_slot' => 'Add Time Slot',
                    'daily_tracking' => [
                        'count' => 'Usage Count Today',
                    ],
                    'collected_day' => 'Collected Today',
                ],
            ],
            'is_percentage' => [
                'fixed' => 'Fixed Amount',
                'percent' => 'Percentage',
            ],
            'status' => [
                'active' => 'Active',
                'inactive' => 'Inactive',
            ],
            'validation' => [
                'start_before_end' => 'The start date must be before the end date.',
                'end_after_start' => 'The end date must be after the start date.',
            ],
            'gift' => [
                'action_label' => 'Gift Coupon',
                'modal_heading' => 'Gift a Personal Coupon to Customer',
                'fields' => [
                    'template' => 'Select Coupon Template',
                    'discount_value' => 'Discount Value',
                    'is_percentage' => 'Type',
                    'is_percentage_percent' => 'Percentage (%)',
                    'is_percentage_fixed' => 'Fixed (VND)',
                    'max_discount' => 'Max Discount',
                    'max_discount_helper' => 'Only applies when type is percentage',
                    'days_valid' => 'Validity Days',
                    'days_valid_suffix' => 'days',
                ],
                'success_title' => 'Coupon Gifted',
                'success_body' => 'Coupon :code has been gifted to the customer.',
                'error_title' => 'Gift Failed',
                'error_template_not_found' => 'Coupon template not found.',
            ],
        ],
        'page_static' => [
            'navigation_label' => 'Static Page',
            'label' => 'Static Page',
            'title' => 'Title',
            'slug' => 'Slug',
            'content' => 'Content',
            'og_image' => 'Image',
            'meta_title' => 'Meta Title',
            'meta_description' => 'Meta Description',
            'meta_keywords' => 'Meta Keywords',
            'is_active' => 'Status',
        ],
        'banner' => [
            'label' => 'Banner',
            'navigation_label' => 'Banner Management',
            'fields' => [
                'order' => 'Sort Order',
                'is_active' => 'Status',
                'image_url' => [
                    'label' => 'Image',
                    'vi' => 'Vietnamese Banner',
                    'en' => 'English Banner',
                    'cn' => 'Chinese Banner',
                ],
                'type' => 'Type',
            ],
            'type' => [
                'HOMEPAGE' => 'Homepage',
                'AFFILIATE_CUSTOMER' => 'Affiliate Customer Page',
                'AFFILIATE_KTV' => 'Affiliate KTV Page',
                'AFFILIATE_AGENCY' => 'Affiliate Agency Page',
            ],
        ],
        'static_contract' => [
            'label' => 'Static Contract',
            'navigation_label' => 'Static Contract Management',
            'label_plural' => 'Static Contracts',
            'fields' => [
                'contract' => 'Contract',
                'note' => [
                    'vi' => 'Vietnamese Note',
                    'en' => 'English Note',
                    'cn' => 'Chinese Note',
                    'label' => 'Note',
                ],
                'created_at' => 'Created At',
                'title' => 'Title',
                'slug' => 'Slug',
                'path' => [
                    'vi' => 'Vietnamese File',
                    'en' => 'English File',
                    'cn' => 'Chinese File',
                    'label' => 'File',
                    'description' => 'Upload PDF file',
                ],
                'type' => 'Type',
            ],
            'errors' => [
                'title_unique' => 'Title already exists',
                'title_max' => 'Title max 255 characters',
                'note_max' => 'Note max 255 characters',
                'slug_unique' => 'Slug already exists',
                'slug_max' => 'Slug max 255 characters',
                'path_required' => 'Please select a file',
                'path_max' => 'File max 10MB',
                'type_required' => 'Please select a type',
                'type_unique' => 'Type already exists',
                'path_pdf_only' => 'Only PDF format is accepted.',
            ],
        ],
        'chat_room' => [
            'label' => 'Chat Room',
            'navigation_label' => 'Chat Room',
            'label_plural' => 'Chat Room',
            'fields' => [
                'customer_avatar' => 'Customer Avatar',
                'customer_name' => 'Customer Name',
                'ktv_avatar' => 'Technician Avatar',
                'ktv_name' => 'Technician Name',
                'last_message' => 'Last Message',
                'started' => 'Started',
                'chat_room' => 'Chat Room',
                'chat' => 'Chat',
                'user' => 'User',
                'ktv' => 'Technician',
                'load_older_messages' => 'Load older messages',
            ],
        ],
        'mobile_notification' => [
            'label' => 'Mobile Notification',
            'navigation_label' => 'Mobile Notification',
            'label_plural' => 'Mobile Notification',
            'fields' => [
                'title' => 'Title',
                'body' => 'Body',
                'user' => 'User',
                'status' => 'Status',
                'description' => 'Description',
                'created_at' => 'Created At',
                'updated_at' => 'Updated At',
                'title_vi' => 'Vietnamese Title',
                'title_en' => 'English Title',
                'title_cn' => 'Chinese Title',
                'description_vi' => 'Vietnamese Description',
                'description_en' => 'English Description',
                'description_cn' => 'Chinese Description',
            ],
            'status' => [
                'pending' => 'Pending',
                'sent' => 'Sent',
                'failed' => 'Failed',
                'read' => 'Read',
            ],
            'action' => [
                'create' => 'Create Notification',
                'modal_description' => 'Fill in the information to create a notification',
                'success_notification_title' => 'Notifications are being sent to users gradually.',
            ],
            'receiver' => [
                'label' => 'Receiver',
                'all' => 'All',
                'client' => 'Client',
                'ktv' => 'KTV',
                'agency' => 'Agency',
            ],
            'validation_messages' => [
                'required' => 'Please enter information',
                'max_length' => 'Maximum length is :max characters',

            ],
        ],
        'date_range' => [
            'day' => 'Today',
            'week' => 'This Week',
            'month' => 'This Month',
            'quarter' => 'This Quarter',
            'year' => 'This Year',
            'all' => 'All Time',
        ],
        'admin_role' => [
            'ADMIN' => 'Administrator',
            'CUSTOMER_SUPPORT' => 'Customer Support',
            'SUPER_ADMIN' => 'Superadmin',
        ],
        'admin_user' => [
            'label' => 'Staff',
        ],
        'staff' => [
            'label' => 'Staff',
            'navigation_label' => 'Human Resources',
            'section' => [
                'account' => 'Account information',
            ],
            'empty_state' => [
                'heading' => 'No staff yet',
                'description' => 'Create the first staff account to manage operations.',
            ],
        ],
        'support_ticket' => [
            'label' => 'Support ticket',
            'navigation_label' => 'Support tickets',
            'status' => [
                'pending' => 'Pending',
                'assigned' => 'Assigned',
                'in_progress' => 'In progress',
                'closed' => 'Closed',
            ],
            'sla_status' => ['ok' => 'Within SLA', 'warning' => 'At risk', 'breached' => 'Breached'],
            'section' => [
                'info' => 'Ticket info',
                'manage' => 'Assign and handle',
                'sla' => 'SLA policy and timeline',
            ],
            'fields' => [
                'customer' => 'Customer',
                'customer_phone' => 'Phone number',
                'category' => 'Support category',
                'assigned_staff' => 'Assigned staff',
                'status' => 'Status',
                'room_id' => 'Chat room ID',
                'latest_booking' => 'Latest booking',
                'last_message_at' => 'Last message',
                'latest_message' => 'Latest message',
                'sla' => 'SLA',
                'sla_hint' => 'Warning after 5 minutes without a first response; breached after 15 minutes and returned to the shared queue.',
                'sla_warning_at' => 'SLA warning time',
                'sla_breached_at' => 'SLA breach time',
                'assigned_at' => 'Assigned at',
                'first_response_at' => 'First response at',
                'closed_at' => 'Closed at',
            ],
            'empty_state' => [
                'heading' => 'No support tickets yet',
                'description' => 'Tickets from mobile will appear here for staff to handle.',
            ],
        ],
        'support_monitor' => [
            'conversations' => 'Conversation monitor',
            'conversation_detail' => 'Full conversation',
            'ticket' => 'Ticket',
            'audit' => 'Handling history',
            'system' => 'System',
            'no_audit' => 'No handling history yet',
            'ticket_not_found' => 'Ticket not found',
            'load_older_messages' => 'Load older messages',
            'performance' => 'Support performance',
            'active_tickets' => 'Active queue',
            'conversation' => 'Conversation',
            'select_ticket' => 'Select a ticket to view the conversation',
            'anonymous' => 'Anonymous customer',
            'support' => 'Support',
            'unassigned' => 'Unassigned',
            'no_active' => 'No active tickets',
            'no_messages' => 'No messages yet',
            'date' => 'Work date',
            'period' => 'Report period',
            'period_day' => 'Daily',
            'period_week' => 'Weekly',
            'period_month' => 'Monthly',
            'staff' => 'Support staff',
            'presence' => 'Presence',
            'online' => 'Online',
            'offline' => 'Offline',
            'all_staff' => 'All staff',
            'performance_table' => 'Performance report',
            'sla_legend' => 'SLA guide: ',
            'sla_legend_description' => 'warning at minute 5 without a first response; breach at minute 15 and return to the shared queue.',
            'received' => 'Received',
            'claimed' => 'Claimed',
            'active' => 'Active',
            'closed' => 'Closed',
            'messages' => 'Messages',
            'unread' => 'Unread',
            'avg_response' => 'First response',
            'avg_processing' => 'Avg. handling time',
            'reassigned' => 'Reassigned',
            'sla_warning_breached' => 'SLA warning/breach',
            'sla_rate' => 'Within SLA',
            'no_staff' => 'No support staff found',
        ],
        'urgency_level' => [
            'need_now' => 'Need Now (30-60 mins)',
            'today' => 'Today',
            'scheduled' => 'Scheduled',
        ],
        'service_request_status' => [
            'new' => 'New',
            'assigned' => 'Assigned to CSKH',
            'searching_ktv' => 'Searching KTV',
            'proposal_sent' => 'Proposal Sent',
            'waiting_customer_confirm' => 'Waiting Customer',
            'matched' => 'Matched',
            'booking_created' => 'Booking Created',
            'closed' => 'Closed',
            'canceled' => 'Canceled',
        ],
        'proposal_status' => [
            'proposed' => 'Proposed',
            'ktv_accepted' => 'KTV Accepted',
            'ktv_declined' => 'KTV Declined',
            'customer_accepted' => 'Customer Accepted',
            'customer_declined' => 'Customer Declined',
            'expired' => 'Expired',
        ],
        'service_request' => [
            'nav' => 'Service Requests & Matching',
            'label' => 'Service Request',
            'plural' => 'Service Requests',
            'fields' => [
                'customer' => 'Customer',
                'cskh' => 'Assigned CSKH',
                'service' => 'Requested Service',
                'techniques' => 'Requested Techniques',
                'address' => 'Service Location',
                'preferred_date' => 'Preferred Date',
                'time_slot' => 'Time Slot',
                'urgency_level' => 'Urgency Level',
                'status' => 'Status',
                'note' => 'Customer Notes',
                'ward' => 'Ward / Commune',
                'province' => 'Province / City',
                'cskh_notes' => 'CSKH Notes',
                'propose_ktv' => 'Propose KTV',
                'create_booking' => 'Create Booking',
                'assign_me' => 'Assign to Me',
            ],
            'action' => [
                'recommend_ktv' => 'Select Recommended KTV (Filtered by Customer Area)',
                'invite_history' => 'Recommendation History',
            ],
            'modal' => [
                'request_status' => 'Request Status',
                'proposed_ktv' => 'Proposed KTV',
                'proposed_by_cskh' => 'Proposed by CSKH',
                'response_status' => 'Response Status',
                'time' => 'Time',
                'no_proposals' => 'No KTV proposal history for this request yet.',
                'close' => 'Close',
            ],
            'messages' => [
                'create_success' => 'Service request submitted successfully! CSKH will contact you soon.',
                'propose_success' => 'KTV proposal sent successfully.',
                'respond_success' => 'Proposal response updated successfully.',
                'booking_created_success' => 'New booking auto-created from service request.',
                'already_matched' => 'This request has already been matched or converted to a booking.',
                'proposal_not_found' => 'No KTV proposal has been sent or accepted for this request yet.',
            ],
        ],
        'invitation_status' => [
            'pending' => 'Pending Response',
            'accepted' => 'Customer Accepted',
            'declined' => 'Customer Declined',
            'expired' => 'Expired',
            'canceled_by_admin' => 'Canceled by CSKH',
        ],
        'proactive_invite' => [
            'nav' => 'KTV Proactive Matching',
            'label' => 'KTV Invitation',
            'plural' => 'KTV Invitations',
            'fields' => [
                'ktv' => 'Inviting KTV',
                'customer' => 'Customer',
                'status' => 'Status',
                'note' => 'KTV Message',
                'expires_at' => 'Expires At',
                'created_at' => 'Sent Time',
                'cancel_invite' => 'Cancel Invitation (CSKH)',
            ],
            'messages' => [
                'send_success' => 'Direct invitation sent successfully!',
                'invite_limit_reached' => 'You reached the maximum limit of 3 pending invitations. Please wait for customer response!',
                'cooldown_active' => 'Customer just declined your invite. Please try again after 60 minutes!',
                'invite_not_found' => 'Invitation not found or expired.',
                'invite_canceled' => 'Invitation canceled successfully.',
            ],
        ],
    ];
