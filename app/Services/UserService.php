    ...
    public function saveAddress(array $data): ServiceReturn
    {
        ...
        try {
            ...
            if (isset($data['is_primary'])) {
                $preparedData['is_primary'] = $data['is_primary'];
            }
            ...
            if (isset($data['latitude'])) {
                $preparedData['latitude'] = $data['latitude'];
            }

            if (isset($data['longitude'])) {
                $preparedData['longitude'] = $data['longitude'];
            }

            if (isset($data['desc'])) {
                $preparedData['desc'] = $data['desc'];
            }

            $userAddress = $this->userAddressRepository->create([
                'user_id' => $data['user_id'],
                'address' => $preparedData['address'] ?? '',
                'latitude' => $preparedData['latitude'] ?? 0,
                'longitude' => $preparedData['longitude'] ?? 0,
                'desc' => $preparedData['desc'] ?? '',
                'is_primary' => $preparedData['is_primary'] ?? false
            ]);
            if (isset($data['is_primary']) && $data['is_primary']) {
                $this->userAddressRepository->query()->where('user_id', $data['user_id'])->update([
                    'is_primary' => false
                ]);
            }
            DB::commit();
