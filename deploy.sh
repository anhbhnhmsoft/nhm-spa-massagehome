...
echo -e "\n${YELLOW}📥 [1/6] Đang kéo code mới từ Git...${NC}"
...
if [ -f "composer.json" ]; then
    echo -e "\n${YELLOW}📦 [2/6] Tối ưu hóa Composer Dependencies...${NC}"
    composer install --no-dev --prefer-dist --optimize-autoloader --no-interaction --quiet
fi

# 3. Chạy Migration database
echo -e "\n${YELLOW}🗄️ [3/6] Chạy Database Migrations...${NC}"
php artisan migrate --force

# 4. Tối ưu hóa & Cache hệ thống
echo -e "\n${YELLOW}⚡ [4/6] Dọn dẹp và làm mới Cache hệ thống...${NC}"
...
echo -e "\n${YELLOW}🔄 [5/6] Khởi động lại Queue Workers...${NC}"
php artisan queue:restart

# 6. Khởi động lại Socket Server (PM2) & PHP-FPM
echo -e "\n${YELLOW}🌐 [6/6] Khởi động lại Socket Server & PHP-FPM...${NC}"
